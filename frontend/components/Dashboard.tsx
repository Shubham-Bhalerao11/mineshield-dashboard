...
const nodes: NodeRecord[] = surfaceNodes.flatMap((node) => [
  { ...node, depth: "surface" as const },
  { ...node, id: `${node.id}′`, depth: "underground" as const, status: node.zone === "Red" ? "Critical" : node.status, sensor: node.sensor === "Crack + UWB" ? "Crack + UWB" : "Tilt + Vibration" },
]);
const nodeGroups = surfaceNodes.map((node, index) => ({
  id: index + 1,
  label: `Group ${index + 1}`,
  anchor: node.id,
  nodes: nodes.filter((candidate) => candidate.id === node.id || candidate.id === `${node.id}′`),
}));
const workers = [
...
const meshNodes: MeshNode[] = surfaceNodes.map((node, index) => ({
...
const predictionSeries = [
  { time: "Now", risk: 34, threshold: 80 },
  { time: "+6h", risk: 39, threshold: 80 },
  { time: "+12h", risk: 47, threshold: 80 },
  { time: "+18h", risk: 58, threshold: 80 },
  { time: "+24h", risk: 68, threshold: 80 },
  { time: "+30h", risk: 79, threshold: 80 },
  { time: "+36h", risk: 88, threshold: 80 },
  { time: "+42h", risk: 94, threshold: 80 },
];
function PredictionGraph() {
  return <section className="proto-panel prediction-panel"><div className="panel-title"><div><b>AI predicted mine subsidence time</b><small>Historical displacement and crack patterns · derived estimate</small></div><span className="prediction-badge">EST. 36–42 HOURS</span></div><div className="prediction-chart"><ResponsiveContainer width="100%" height={250}><AreaChart data={predictionSeries}><defs><linearGradient id="predictionFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ef6b72" stopOpacity={.42} /><stop offset="100%" stopColor="#ef6b72" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="#21394a" strokeDasharray="3 3" /><XAxis dataKey="time" stroke="#809aaa" fontSize={11} /><YAxis domain={[0, 100]} unit="%" stroke="#809aaa" fontSize={11} /><Tooltip contentStyle={{ background: "#0e1d2d", border: "1px solid #2b4a5d", borderRadius: 8 }} /><Area type="monotone" dataKey="risk" name="Predicted risk" stroke="#ff7c7a" fill="url(#predictionFill)" strokeWidth={2.5} /><Line type="monotone" dataKey="threshold" name="Action threshold" stroke="#f7c35c" strokeDasharray="5 5" dot={false} /></AreaChart></ResponsiveContainer></div><div className="prediction-legend"><span><i className="prediction-risk" /> Predicted subsidence risk</span><span><i className="prediction-threshold" /> Action threshold</span><b>Confidence 82%</b></div></section>;
}
...
export default function Dashboard() {
  ...
  const selected = surfaceNodes.find((node) => node.id === selectedNode) ?? surfaceNodes[0];
...
function Nodes({ selected, onSelect }: { selected: string; onSelect: (id: string) => void }) {
  ...
  return <><PageHeading title="Nodes" subtitle="System architecture grouped by surface node and its underground counterpart." action={<button className="secondary-action" onClick={() => onSelect(selected)}><RefreshCw size={14} /> Refresh</button>} /><section className="node-groups" aria-label="Node groups">{nodeGroups.map((group) => { const isOpen = expandedGroup === group.id; return <article className={`proto-panel node-group ${isOpen ? "open" : ""}`} key={group.id}><button className="node-group-toggle" onClick={() => setExpandedGroup(isOpen ? 0 : group.id)} aria-expanded={isOpen}><span className="group-number">{String(group.id).padStart(2, "0")}</span><span><b>{group.label}</b><small>{group.anchor} surface ↔ {group.anchor}′ underground · {group.nodes[0].zone} zone</small></span><Pill zone={group.nodes[0].zone}>{group.nodes[0].status}</Pill><ChevronDown size={18} /></button>{isOpen && <div className="node-architecture"><div className="architecture-layer"><span className="layer-label">ABOVE GROUND</span><NodeDetail node={group.nodes[0]} index={group.id * 2 - 2} selected={selected} onSelect={onSelect} lastSeen={lastSeen} /></div><div className="architecture-connector"><i /><span>VERTICAL MESH LINK</span><i /></div><div className="architecture-layer underground"><span className="layer-label">BELOW GROUND</span><NodeDetail node={group.nodes[1]} index={group.id * 2 - 1} selected={selected} onSelect={onSelect} lastSeen={lastSeen} /></div></div>}</article>; })}</section></>;
}
function NodeDetail({ node, index, selected, onSelect, lastSeen }: { node: NodeRecord; index: number; selected: string; onSelect: (id: string) => void; lastSeen: (index: number) => string }) {
  return <button className={`node-detail ${selected === node.id ? "selected" : ""}`} onClick={() => onSelect(node.id)}><div className="node-detail-header"><div><b>{node.id}</b><small>{node.depth === "surface" ? "Surface relay" : "Underground sensor"} · seq {1861 - index * 17}</small></div><span className={`status-dot ${node.zone.toLowerCase()}`} /> {node.status}</div><div className="node-detail-grid"><span><small>Zone</small><b>{node.zone}</b></span><span><small>Last seen</small><b>{lastSeen(index)}</b></span><span><small>Crack detector</small><b className="connected">Connected</b></span><span><small>UWB</small><b className="connected">Connected</b></span><span><small>Children vibration</small><b className="connected">Connected</b></span><span><small>Humidity</small><b className="connected">Connected</b></span></div></button>;
}
